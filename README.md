...
#### Cloudflare Pages自動デプロイ（CI成功後制御）
CI成功後にapps/appのビルド成果物をCloudflare Pagesに自動デプロイします。品質検証をパスしたコードのみがデプロイされる仕組みを採用しています。

**デプロイ機能**:
- **PRプレビューデプロイ**: CI成功後に自動的にプレビュー環境を構築
- **本番デプロイ**: CI成功後にmainブランチから本番環境へ自動デプロイ
- **CI/デプロイ分離**: CIとデプロイワークフローを分離し、品質ゲート機能を実現
...
**PRでの確認プロセス（CI/デプロイ分離）**:
1. PR作成時にCIワークフローが自動実行される（リンティング・テスト・ビルドチェック）
2. **CI成功後**に自動的にCloudflare Pagesデプロイワークフローが実行される
3. アプリケーションがビルドされ、Cloudflare Pagesにデプロイされる
4. PRに自動コメントでプレビューURLとデプロイ情報が投稿される
5. プレビューURLから即座にアプリケーションを確認可能
6. メインブランチにマージ時も同様にCI成功後に本番環境へ自動デプロイされる

**CI/デプロイ分離のメリット**:
- **品質保証**: リンティング・テスト・ビルドエラーがある場合はデプロイされない
- **効率性**: 品質に問題のあるコードの無駄なデプロイを防止
- **信頼性**: 常に品質検証済みのコードのみが本番環境に反映される
