...
describe('Cloudflare Pages デプロイワークフロー設定', () => {
  const workflowPath = join(process.cwd(), '../../.github/workflows/pages-preview.yml')
  ...
  it('Cloudflare Pagesデプロイステップが正しく設定されている', () => {
    ...
    expect(deployStep.with.command).toContain('--project-name=study-github-agent')
  ...
  it('PRコメントステップが正しく条件付きである', () => {
    ...
    expect(commentStep.with.body).toContain('${{ steps.deploy.outputs.deployment-url }}')
  })
})